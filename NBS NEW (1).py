...
from PyQt6.QtWidgets import (
    ...
    QFileDialog, QCheckBox, QStackedWidget, QSizePolicy, QCompleter, QGroupBox, QMenu, QGraphicsColorizeEffect, QListWidgetItem
)
...
from PyQt6.QtCore import Qt, QDate, QSizeF, QMarginsF, QPoint, pyqtSignal, QUrl


DB_NAME = "nbs.db"
ENTRY_TYPES = ["Income", "Expense", "Capital"]
INCOME_CATEGORIES = ["Sales", "Services"]
PAYROLL_TYPES = ["Salary Payment", "Advance", "Deduction"]
...
class PayrollTab(QWidget):
    def __init__(self):
        ...
        self.tx_type.addItems(PAYROLL_TYPES)
    ...
    def record_transaction(self):
        ...
        amt = parse_amount(self.tx_amount.text())
        if amt <= 0:
            QMessageBox.warning(self, "Amount is required.", "Please enter a valid amount.")
            return
...
class VendorsTab(QWidget):
    ...
    def __init__(self, daily_tab=None):
        ...
        self.export_pdf_btn = QPushButton("Print")
    ...
    def export_transactions_pdf(self):
       ...
       # 1. Preview dialog with QWebEngineView
        ...
        def handle_print():
            printer = QPrinter(QPrinter.PrinterMode.HighResolution)
            printer.setPageSize(QPageSize(QPageSize.PageSizeId.A4))
            printer.setPageMargins(QMarginsF(36, 30, 36, 30), QPageLayout.Unit.Millimeter)
            printer.setPageOrientation(QPageLayout.Orientation.Portrait)
            dialog = QPrintDialog(printer, self)
            if dialog.exec():
                view.page().print(printer, lambda ok: (
                    QMessageBox.information(self, "Print", "Print sent to printer.") if ok else
                    QMessageBox.warning(self, "Print", "Failed to print.")
                ))
...
class SettingsTab(QWidget):
    ...
    """    
        ...
        # Export using QWebEngineView (headless, no dialog)
        from PyQt6.QtWebEngineWidgets import QWebEngineView
